...
The database must be local to the gateway host. Do not place `gateway.sqlite3` on
...
```text
FBS_GATEWAY_DB_PATH=/var/lib/fbs-interlock-gateway/gateway.sqlite3
...
`gateway.sqlite3` beside the supplied `config.yaml`; that directory is already
...
Shelly passwords stored in `gateway.sqlite3` remain recoverable by an account that can
...
`gateway.sqlite3`, configuration exports containing credentials, or generated
